                    ...
                    unsigned char *sendbuf, size_t sendbuf_size, unsigned char *readbuf, size_t readbuf_size)
{
    ...
    {
        c->messageHandlers[i].topicFilter = (char*)malloc(ezdev_sdk_recv_topic_len);
        memset(c->messageHandlers[i].topicFilter, 0, ezdev_sdk_recv_topic_len);
    }
...
static int readPacket(MQTTClient *c, Timer *timer)
{
    ...
    {
        ezdev_sdk_kernel_log_error(mkernel_internal_net_socket_error, 0, "size range, rem_len:%d\n", rem_len);
        MQTTNetSetLastError(mkernel_internal_net_socket_error);
        goto exit;
    }
    ...
    if (rem_len > 0 && (c->ipstack->mqttread(c->ipstack, c->readbuf + len, rem_len, 5000 /*TimerLeftMS(timer)*10*/) != rem_len)) 
    {
        ...
        //rc = header.bits.type;
		ezdev_sdk_kernel_log_error(mkernel_internal_net_socket_error, 0, "rev rem packet, rem_len = %d\n", rem_len);
        MQTTNetSetLastError(mkernel_internal_net_socket_error);
        goto exit;
    }
...
int keepalive(MQTTClient *c)
{
    ...
    if (TimerIsExpired(&c->ping_timer))
    {
        ...
        ezdev_sdk_kernel_log_debug(0, 0, "heartbeat ping");
...
int cycle(MQTTClient *c, Timer *timer)
{
    ...
    {
		ezdev_sdk_kernel_log_debug(0, 0, "heartbeat clear, packet_type:%d", packet_type);
...
int MQTTYield(MQTTClient *c, int timeout_ms)
{
    ...
    //  	do //先去除循环，防止utc时间修改导致在一段时间内循环无法退出
    //      {
...
int waitfor(MQTTClient *c, int packet_type, Timer *timer)
{
    ...
    do
    {
        ...
        {
            rc = FAILURE;
            ezdev_sdk_kernel_log_warn(rc, MQTTNetGetLastError(), "waitfor err");
            break;
        }
    } while (1);
...
int MQTTSubscribe(MQTTClient *c, const char *topicFilter, enum QoS qos, messageHandler messageHandler)
{
    ...
    if (waitfor(c, SUBACK, &timer) == SUBACK) // wait for suback
    {
        ...
        if (rc != 0x80)
        {
            ...
            for (i = 0; i < MAX_MESSAGE_HANDLERS; ++i)
            {
                if (c->messageHandlers[i].topicFilter != 0 && strcmp(topicFilter, c->messageHandlers[i].topicFilter) == 0)
                {
                    rc = 0;
                    break;
                }

                if (strlen(c->messageHandlers[i].topicFilter) == 0)
                {
                    sprintf(c->messageHandlers[i].topicFilter, "%s", topicFilter);
...
int MQTTPublish(MQTTClient *c, const char *topicName, MQTTMessage *message)
{
    ...
    {
        int packetTypeWait4 = (message->qos == QOS1) ? PUBACK : PUBCOMP;
