...
ez_err_e httpd_sess_new(struct httpd_data *hd, int newfd)
{
    ez_log_v(TAG, LOG_FMT("fd = %d"), newfd);

    if (httpd_sess_get(hd, newfd))
    {
        ez_log_e(TAG, LOG_FMT("session already exists with fd = %d"), newfd);
    ...
    for (i = 0; i < hd->config.max_open_sockets; i++)
    {
        if (hd->hd_sd[i].fd == -1)
        {
            ...
            {
                ez_err_e ret = hd->config.open_fn(hd, hd->hd_sd[i].fd);
    ...
    ez_log_v(TAG, LOG_FMT("unable to launch session for fd = %d"), newfd);
    return ez_errno_fail;
}
...
void httpd_sess_delete_invalid(struct httpd_data *hd)
{
    for (int i = 0; i < hd->config.max_open_sockets; i++)
    {
        ...
        {
            ez_log_w(TAG, LOG_FMT("Closing invalid socket %d"), hd->hd_sd[i].fd);
...
/* This MUST return ez_errno_succ on successful execution. If any other
 * value is returned, everything related to this socket will be
 * cleaned up and the socket will be closed.
 */
ez_err_e httpd_sess_process(struct httpd_data *hd, int newfd)
{
    ...
    ez_log_v(TAG, LOG_FMT("httpd_req_new"));
    if (httpd_req_new(hd, sd) != ez_errno_succ)
    {
        return ez_errno_fail;
    }
    ez_log_v(TAG, LOG_FMT("httpd_req_delete"));
    if (httpd_req_delete(hd) != ez_errno_succ)
    {
        return ez_errno_fail;
    }
    ez_log_v(TAG, LOG_FMT("success"));
...
ez_err_e httpd_sess_close_lru(struct httpd_data *hd)
{
    ...
    ez_log_v(TAG, LOG_FMT("fd = %d"), lru_fd);
