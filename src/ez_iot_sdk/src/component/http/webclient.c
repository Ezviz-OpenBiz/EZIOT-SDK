...
static int webclient_read_line(struct webclient_session *session, char *buffer, int size)
{
    ...
    if (count > size)
    {
        ez_log_e(DBG_TAG,"read line failed. The line data length is out of buffer size(%d)!", count);
                                     ...
                                     const char *url, const char **request)
{
    ...
    *request = path_ptr ? path_ptr : "/";
    ...
    ez_log_d(DBG_TAG,"host address: %s , port: %s", session->host, port_str);
    ...
    /* resolve the host name. */
    {
        ...
        if (ret != 0)
        {
            ez_log_e(DBG_TAG,"getaddrinfo err: %d '%s'.", ret, session->host);
            rc = -WEBCLIENT_ERROR;
            goto __exit;
        }
    }
...
static int webclient_open_tls(struct webclient_session *session, const char *URI)
{
    ...
    {
        ez_log_e(DBG_TAG,"no memory for tls_session buffer!");
        return -WEBCLIENT_ERROR;
    }

    if ((tls_ret = mbedtls_client_init(session->tls_session, (void *)pers, strlen(pers))) < 0)
    {
        ez_log_e(DBG_TAG,"initialize https client failed return: -0x%x.", -tls_ret);
        return -WEBCLIENT_ERROR;
    }
...
static int webclient_connect(struct webclient_session *session, const char *URI)
{
    ...
    timeout.tv_usec = 0;
#else
    int timeout = WEBCLIENT_DEFAULT_TIMEO * 1000;
#endif
    ...
    if (strncmp(URI, "https://", 8) == 0)
    {
        ...
        {
            ez_log_e(DBG_TAG,"connect failed, https client open URI(%s) failed!", URI);
            return -WEBCLIENT_ERROR;
        }
        session->is_tls = RT_TRUE;
#else
        ez_log_e(DBG_TAG,"not support https connect, please enable webclient https configure!");
        rc = -WEBCLIENT_ERROR;
        goto __exit;
#endif
    }
    ...
    if (rc != WEBCLIENT_OK)
    {
        ez_log_e(DBG_TAG,"connect failed, resolve address error(%d).", rc);
        goto __exit;
    }
    ...
    {
        ez_log_e(DBG_TAG,"connect failed, resolve request address error.");
        rc = -WEBCLIENT_ERROR;
        goto __exit;
    }
    ...
    if (session->tls_session)
    {
        ...
        {
            ez_log_e(DBG_TAG,"connect failed, https client context return: -0x%x", -tls_ret);
            return -WEBCLIENT_ERROR;
        }

        if ((tls_ret = mbedtls_client_connect(session->tls_session)) < 0)
        {
            ez_log_e(DBG_TAG,"connect failed, https client connect return: -0x%x", -tls_ret);
...

    {
        ...
        if (socket_handle < 0)
        {
            ez_log_e(DBG_TAG,"connect failed, create socket(%d) error.", socket_handle);
        ...
        if (connect(socket_handle, res->ai_addr, res->ai_addrlen) != 0)
        {
            ...
            ez_log_e(DBG_TAG,"connect failed, connect socket(%d) error.", socket_handle);
...
int webclient_header_fields_add(struct webclient_session *session, const char *fmt, ...)
{
    ...
    if (length < 0)
    {
        ez_log_e(DBG_TAG,"add fields header data failed, return length(%d) error.", length);
        return -WEBCLIENT_ERROR;
    }
    va_end(args);
    ...
    {
        ez_log_e(DBG_TAG,"not enough header buffer size(%ld)!", session->header->size);
        return -WEBCLIENT_ERROR;
    }
...
static int webclient_send_header(struct webclient_session *session, int method)
{
    ...
    else
    {
        if (method != WEBCLIENT_USER_METHOD)
        {
            ...
            if (memcmp(header, "HTTP/1.", rt_strlen("HTTP/1.")))
            {
                ...
                {
                      ez_log_e(DBG_TAG,"no memory for header buffer!");
                    rc = -WEBCLIENT_NOMEM;
                    goto __exit;
                }
            ...
            if (session->header->length > session->header->size)
            {
                ez_log_e(DBG_TAG,"send header failed, not enough header buffer size(%ld)!", session->header->size);
    ...
    /* get and echo request header data */
    {
        ...
        int header_line_len;
        ez_log_d(DBG_TAG,"request header:");
        ...
        for (header_str = session->header->buffer; (header_ptr = rt_strstr(header_str, "\r\n")) != RT_NULL;)
        {
            ...
            {
                ez_log_d(DBG_TAG,"header_line_len:%d, header_str:%s", header_line_len, header_str);
            }
...
int webclient_handle_response(struct webclient_session *session)
{
    ...
    while (1)
    {
        ...
        ez_log_d(DBG_TAG,"%s", mime_buffer);
    ...
    if (mime_ptr == RT_NULL)
    {
        ez_log_e(DBG_TAG,"no memory for get http status code buffer!");
        return -WEBCLIENT_NOMEM;
    }
    ...
    if (webclient_header_fields_get(session, "Content-Length") != RT_NULL)
    {
        ...
        ez_log_e(DBG_TAG, "Content-Length got: %d", session->content_length);
    }
    else
    {
        ez_log_e(DBG_TAG, "Content-Length not found.");
    }
...
struct webclient_session *webclient_session_create(size_t header_sz)
{
    ...
    if (session == RT_NULL)
    {
        ez_log_e(DBG_TAG,"webclient create failed, no memory for webclient session!");
        return RT_NULL;
    }
    ...
    {
        ez_log_e(DBG_TAG,"webclient create failed, no memory for session header!");
        web_free(session);
        session = RT_NULL;
        return RT_NULL;
    }
    ...
    {
        ez_log_e(DBG_TAG,"webclient create failed, no memory for session header buffer!");
...
int webclient_get(struct webclient_session *session, const char *URI)
{
    ...
    ez_log_d(DBG_TAG,"get position handle response(%d).", resp_status);
...
int webclient_post(struct webclient_session *session, const char *URI, const void *post_data, size_t data_len)
{
    ...
    {
         ez_log_e(DBG_TAG,"input post data length failed");
        return -WEBCLIENT_ERROR;
    }
    ...
    if (post_data && (data_len > 0))
    {
        ...
        resp_status = webclient_handle_response(session);
        ez_log_d(DBG_TAG,"post handle response(%d).", resp_status);
    }
...
int webclient_set_timeout(struct webclient_session *session, int millisecond)
{
    ...
    /* set recv timeout option */
    int ret = setsockopt(session->socket, SOL_SOCKET, SO_RCVTIMEO,
               (void *)&timeout, sizeof(timeout));
    ret = setsockopt(session->socket, SOL_SOCKET, SO_SNDTIMEO,
               (void *)&timeout, sizeof(timeout));
    ...
    do
    {
        ...
        if (bytes_read <= 0)
        {
...
#endif
            ez_log_d(DBG_TAG,"receive data error(%d).", bytes_read);
            ...
            else
            {
                ...
                {
                    /* recv timeout */
                    ez_log_e(DBG_TAG,"receive data timeout.");
                    return -WEBCLIENT_TIMEOUT;
                }
                else
                {
    ...
    *response = RT_NULL;
    ...
    if (session->content_length < 0)
    {
        ...
        while (1)
        {
            ...
            if (new_resp == RT_NULL)
            {
                ez_log_e(DBG_TAG,"no memory for realloc new response buffer!");
                break;
            }
...
int webclient_request_header_add(char **request_header, const char *fmt, ...)
{
    ...
    if (*request_header == RT_NULL)
    {
        ...
        if (header == RT_NULL)
        {
             ez_log_e(DBG_TAG,"No memory for webclient request header add.");
            return RT_NULL;
        }
    ...
    if (length < 0)
    {
         ez_log_e(DBG_TAG,"add request header data failed, return length(%d) error.", length);
        return -WEBCLIENT_ERROR;
    }
    va_end(args);
    ...
    {
         ez_log_e(DBG_TAG,"not enough request header data size(%d)!", WEBCLIENT_HEADER_BUFSZ);
        return -WEBCLIENT_ERROR;
    }
...
int webclient_request(const char *URI, const char *header, const void *post_data, size_t data_len, void **response, size_t *resp_len)
{
    ...
    {
         ez_log_e(DBG_TAG,"request get failed, get response data cannot be empty.");
        return -WEBCLIENT_ERROR;
    }
    ...
    {
        ez_log_e(DBG_TAG,"input response data or length failed");
        return -WEBCLIENT_ERROR;
    }
 ...
 *
 * @param URI input server address
 * @param filename post data filename，设备没有文件系统，传NULL,linux 传文件所在的相对路径
 * @param form_data  form data
 * @param ts_to_websession  tsdb 读文件写flash 回调函数，linux 传NULL
 *
 * @return <0: POST request failed
 *         =0: success
 */
int webclient_post_file(const char* URI, const char* filename,
        const char* form_data,kv_err_e (*ts_to_websession)())
{
    size_t length = 0;
    char boundary[60];
	...
	if(NULL !=filename)
	{
		/* linux with filesys todo
	    fd = open(filename, O_RDONLY, 0);
	    if (fd < 0)
	    {
	        ez_log_e(TAG_AT,"post file failed, open file(%s) error.", filename);
    ...
    if (buffer == RT_NULL)
    {
        ez_log_e(TAG_AT,"post file failed, no memory for response buffer.");
        rc = -WEBCLIENT_NOMEM;
        goto __exit;
    }
    ...
    if (header == RT_NULL)
    {
        ez_log_e(TAG_AT,"post file failed, no memory for header buffer.");
        rc = -WEBCLIENT_NOMEM;
        goto __exit;
    }
    header_ptr = header;
	...
	length += rt_strlen(boundary) + 80000;   //boundary 方式，文件长度写的比实际长不影响服务器解析
	...
	else if(NULL !=ts_to_websession)
	{
		(*ts_to_websession)( (void *)session);
	}
