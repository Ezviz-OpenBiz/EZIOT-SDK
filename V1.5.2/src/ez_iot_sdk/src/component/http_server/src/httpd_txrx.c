...
ez_err_e httpd_sess_set_send_override(httpd_handle_t hd, int sockfd, httpd_send_func_t send_func)
{
...
ez_err_e httpd_sess_set_recv_override(httpd_handle_t hd, int sockfd, httpd_recv_func_t recv_func)
{
    struct sock_db *sess = httpd_sess_get(hd, sockfd);
    if (!sess)
    {
...
ez_err_e httpd_sess_set_pending_override(httpd_handle_t hd, int sockfd, httpd_pending_func_t pending_func)
{
    struct sock_db *sess = httpd_sess_get(hd, sockfd);
    if (!sess)
    {
...
int httpd_send(httpd_req_t *r, const char *buf, size_t buf_len)
{
    ...
    if (ret < 0)
    {
        ez_log_v(TAG, LOG_FMT("error in send_fn"));
        return ret;
    }
    return ret;
}

static ez_err_e httpd_send_all(httpd_req_t *r, const char *buf, size_t buf_len)
{
    struct httpd_req_aux *ra = r->aux;
    int ret;
    ...
    while (buf_len > 0)
    {
        ...
        ez_log_v(TAG, LOG_FMT("sent = %d"), ret);
        buf += ret;
        buf_len -= ret;
    }
    return ez_errno_succ;
...
{
    ez_log_v(TAG, LOG_FMT("requested length = %d"), buf_len);
    ...
    {
        ez_log_v(TAG, LOG_FMT("pending length = %d"), ra->sd->pending_len);
    ...
    if (ret < 0)
    {
        ez_log_v(TAG, LOG_FMT("error in recv_fn"));
    ...
    ez_log_v(TAG, LOG_FMT("received length = %d"), ret + pending_len);
...
size_t httpd_unrecv(struct httpd_req *r, const char *buf, size_t buf_len)
{
    ...
    ez_log_v(TAG, LOG_FMT("length = %d"), ra->sd->pending_len);
 ...
 */
ez_err_e httpd_resp_set_hdr(httpd_req_t *r, const char *field, const char *value)
{
    ...
    {
        return ez_errno_httpd_resp_hdr;
    }
    ...
    ez_log_v(TAG, LOG_FMT("new header = %s: %s"), field, value);
    return ez_errno_succ;
}
 ...
 */
ez_err_e httpd_resp_set_status(httpd_req_t *r, const char *status)
{
 ...
 */
ez_err_e httpd_resp_set_type(httpd_req_t *r, const char *type)
{
    if (r == NULL || type == NULL)
    {
        return ez_errno_param_invalid;
...
ez_err_e httpd_resp_send(httpd_req_t *r, const char *buf, ssize_t buf_len)
{
    if (r == NULL)
    {
        return ez_errno_param_invalid;
    ...
    if (httpd_send_all(r, ra->scratch, strlen(ra->scratch)) != ez_errno_succ)
    {
        return ez_errno_httpd_resp_send;
    }
    ...
    {
        /* Send header field */
        if (httpd_send_all(r, ra->resp_hdrs[i].field, strlen(ra->resp_hdrs[i].field)) != ez_errno_succ)
        {
            return ez_errno_httpd_resp_send;
        }
        /* Send ': ' */
        if (httpd_send_all(r, colon_separator, strlen(colon_separator)) != ez_errno_succ)
        {
            return ez_errno_httpd_resp_send;
        }
        /* Send header value */
        if (httpd_send_all(r, ra->resp_hdrs[i].value, strlen(ra->resp_hdrs[i].value)) != ez_errno_succ)
        {
            return ez_errno_httpd_resp_send;
        }
        /* Send CR + LF */
        if (httpd_send_all(r, cr_lf_seperator, strlen(cr_lf_seperator)) != ez_errno_succ)
        {
            return ez_errno_httpd_resp_send;
        }
    }
    ...
    if (buf && buf_len)
    {
        if (httpd_send_all(r, buf, buf_len) != ez_errno_succ)
        {
            return ez_errno_httpd_resp_send;
        }
    }
...
ez_err_e httpd_resp_send_chunk(httpd_req_t *r, const char *buf, ssize_t buf_len)
{
    if (r == NULL)
    {
        return ez_errno_param_invalid;
    ...
    if (httpd_send_all(r, len_str, strlen(len_str)) != ez_errno_succ)
    {
        return ez_errno_httpd_resp_send;
    }
    ...
    if (buf)
    {
        if (httpd_send_all(r, buf, (size_t)buf_len) != ez_errno_succ)
        {
            return ez_errno_httpd_resp_send;
        }
    }
...
ez_err_e httpd_resp_send_404(httpd_req_t *r)
{
    return httpd_resp_send_err(r, HTTPD_404_NOT_FOUND);
}

ez_err_e httpd_resp_send_408(httpd_req_t *r)
{
    return httpd_resp_send_err(r, HTTPD_408_REQ_TIMEOUT);
}

ez_err_e httpd_resp_send_500(httpd_req_t *r)
{
    return httpd_resp_send_err(r, HTTPD_500_SERVER_ERROR);
}

ez_err_e httpd_resp_send_err(httpd_req_t *req, httpd_err_resp_t error)
{
    const char *msg;
    const char *status;
    switch (error)
    ...
    ez_log_w(TAG, LOG_FMT("%s - %s"), status, msg);
...
int httpd_req_recv(httpd_req_t *r, char *buf, size_t buf_len)
{
    ...
    if (!httpd_valid_req(r))
    {
        ez_log_w(TAG, LOG_FMT("invalid request"));
        return HTTPD_SOCK_ERR_INVALID;
    }

    struct httpd_req_aux *ra = r->aux;
    ez_log_v(TAG, LOG_FMT("remaining length = %d"), ra->remaining_len);
    ...
    ez_log_v(TAG, LOG_FMT("received length = %d"), ret);
    return ret;
}
...
static int httpd_sock_err(const char *ctx, int sockfd)
{
    ...
    {
        ez_log_e(TAG, LOG_FMT("error calling getsockopt : %d"), errno);
        return HTTPD_SOCK_ERR_FAIL;
    }
    ez_log_w(TAG, LOG_FMT("error in %s : %d"), ctx, sock_err);
