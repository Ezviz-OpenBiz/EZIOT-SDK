...
static ez_err_e verify_url(http_parser *parser)
{
    ...
    {
        ez_log_w(TAG, LOG_FMT("HTTP Operation not supported"));
    ...
    {
        ez_log_w(TAG, LOG_FMT("URI length (%d) greater than supported (%d)"),
                 length, sizeof(r->uri));
    ...
    ez_log_v(TAG, LOG_FMT("received URI = %s"), r->uri);

    /* Make sure version is HTTP/1.1 */
    if ((parser->http_major != 1) && (parser->http_minor != 1))
    {
        ez_log_w(TAG, LOG_FMT("unsupported HTTP version = %d.%d"),
    ...
    {
        ez_log_w(TAG, LOG_FMT("http_parser_parse_url failed with errno = %d"),
                 parser->http_errno);
 ...
 */
static ez_err_e cb_url(http_parser *parser,
                       const char *at, size_t length)
{
    ...
    {
        ez_log_v(TAG, LOG_FMT("message begin"));
    ...
    {
        ez_log_e(TAG, LOG_FMT("unexpected state transition"));
        parser_data->status = PARSING_FAILED;
        return ez_errno_fail;
    }

    ez_log_v(TAG, LOG_FMT("processing url = %.*s"), length, at);
...
static ez_err_e pause_parsing(http_parser *parser, const char *at)
{
    parser_data_t *parser_data = (parser_data_t *)parser->data;
    struct httpd_req *r = parser_data->req;
    struct httpd_req_aux *ra = r->aux;
    ...
    {
        ez_log_e(TAG, LOG_FMT("data too large for un-recv = %d"),
...
static size_t continue_parsing(http_parser *parser, size_t length)
{
    ...
    ez_log_v(TAG, LOG_FMT("skip pre-parsed data of size = %d"), length);

    http_parser_pause(parser, 0);
    data->paused = false;
    ez_log_v(TAG, LOG_FMT("un-paused"));
    return length;
}
 ...
 */
static ez_err_e cb_header_field(http_parser *parser, const char *at, size_t length)
{
    parser_data_t *parser_data = (parser_data_t *)parser->data;
    struct httpd_req *r = parser_data->req;
    struct httpd_req_aux *ra = r->aux;
    ...
    {
        if (verify_url(parser) != ez_errno_succ)
        {
            parser_data->status = PARSING_FAILED;
            return ez_errno_fail;
        }

        ez_log_v(TAG, LOG_FMT("headers begin"));
        ...
        if (pause_parsing(parser, at) != ez_errno_succ)
        {
            parser_data->status = PARSING_FAILED;
            return ez_errno_fail;
        }
    ...
    ez_log_v(TAG, LOG_FMT("processing field = %.*s"), length, at);
 ...
 */
static ez_err_e cb_header_value(http_parser *parser, const char *at, size_t length)
{
    parser_data_t *parser_data = (parser_data_t *)parser->data;
    struct httpd_req *r = parser_data->req;
    struct httpd_req_aux *ra = r->aux;
    ...
    ez_log_v(TAG, LOG_FMT("processing value = %.*s"), length, at);
 ...
 */
static ez_err_e cb_headers_complete(http_parser *parser)
{
    parser_data_t *parser_data = (parser_data_t *)parser->data;
    struct httpd_req *r = parser_data->req;
    struct httpd_req_aux *ra = r->aux;
    ...
    if (parser_data->status == PARSING_URL)
    {
        ez_log_v(TAG, LOG_FMT("no headers"));
        if (verify_url(parser) != ez_errno_succ)
        {
            parser_data->status = PARSING_FAILED;
            return ez_errno_fail;
    ...
    ez_log_v(TAG, LOG_FMT("bytes read     = %d"), parser->nread);
    ez_log_v(TAG, LOG_FMT("content length = %zu"), r->content_len);

    if (parser->upgrade)
    {
        ez_log_w(TAG, LOG_FMT("upgrade from HTTP not supported"));
 ...
 * Will be invoked ONLY once every packet
 */
static ez_err_e cb_on_body(http_parser *parser, const char *at, size_t length)
{
    parser_data_t *parser_data = (parser_data_t *)parser->data;
    ...
    ez_log_v(TAG, LOG_FMT("body begins"));
    return ez_errno_succ;
}
 ...
 * Will be invoked ONLY once every packet
 */
static ez_err_e cb_no_body(http_parser *parser)
{
    parser_data_t *parser_data = (parser_data_t *)parser->data;
    const char *at = parser_data->last.at;
    ...
    parser_data->last.length = 0;
    parser_data->status = PARSING_COMPLETE;
    ez_log_v(TAG, LOG_FMT("message complete"));
    return ez_errno_succ;
}
...
static int read_block(httpd_req_t *req, size_t offset, size_t length)
{
    ...
    if (nbytes < 0)
    {
        ez_log_v(TAG, LOG_FMT("error in httpd_recv"));
    ...
    {
        ez_log_v(TAG, LOG_FMT("connection closed"));
        return -1;
    }

    ez_log_v(TAG, LOG_FMT("received HTTP request block size = %d"), nbytes);
    return nbytes;
}
...
static int parse_block(http_parser *parser, size_t offset, size_t length)
{
    ...
    if (!length)
    {
        ez_log_w(TAG, LOG_FMT("response uri/header too big"));
        switch (data->status)
        {
        case PARSING_URL:
    ...
    {
        ez_log_w(TAG, LOG_FMT("parsing failed"));
        return -1;
    }
    else if (data->paused)
    {
    ...
    else if (nparsed != length)
    {
        ...
        ez_log_w(TAG, LOG_FMT("incomplete (%d/%d) with parser error = %d"),
                 nparsed, length, parser->http_errno);
        return -1;
    }

    /* Continue parsing this section of HTTP request packet */
    ez_log_v(TAG, LOG_FMT("parsed block size = %d"), offset + nparsed);
 ...
 */
static ez_err_e httpd_parse_req(struct httpd_data *hd)
{
    ...
    ez_log_v(TAG, LOG_FMT("parsing complete"));
    return httpd_uri(hd);
}
 ...
 */
ez_err_e httpd_req_new(struct httpd_data *hd, struct sock_db *sd)
{
    httpd_req_t *r = &hd->hd_req;
    ...
    /* Parse request */
    ez_err_e err = httpd_parse_req(hd);
    if (err != ez_errno_succ)
    {
        httpd_req_cleanup(r);
    }
    return err;
 ...
 */
ez_err_e httpd_req_delete(struct httpd_data *hd)
{
    httpd_req_t *r = &hd->hd_req;
    struct httpd_req_aux *ra = r->aux;
    ...
    while (ra->remaining_len)
    {
        ...
        dummy[ret] = '\0';
        ez_log_v(TAG, LOG_FMT("purging data : %s"), dummy);
    }
...
ez_err_e httpd_query_key_value(const char *qry_str, const char *key, char *val, size_t val_size)
{
    ...
    while (strlen(qry_ptr))
    {
        ...
        if (buf_len < val_size)
        {
            return ez_errno_httpd_result_trunc;
        }
        return ez_errno_succ;
    }
    ez_log_v(TAG, LOG_FMT("key %s not found"), key);
...
ez_err_e httpd_req_get_url_query_str(httpd_req_t *r, char *buf, size_t buf_len)
{
    if (r == NULL || buf == NULL)
    {
        return ez_errno_param_invalid;
    ...
    if (!httpd_valid_req(r))
    {
        return ez_errno_httpd_invalid_req;
    }
...
ez_err_e httpd_req_get_hdr_value_str(httpd_req_t *r, const char *field, char *val, size_t val_size)
{
    if (r == NULL || field == NULL)
    {
        return ez_errno_param_invalid;
