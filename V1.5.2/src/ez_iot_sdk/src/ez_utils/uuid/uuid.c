...
void get_ieee_node_identifier(void* node);
...
static void mult32(unsigned32 u, unsigned32 v, unsigned64_t* result)
...
static void get_system_time(unsigned64_t* uuid_time)
{
    struct timeval tp;
...
{
    if ((rand_m += 7) >= 9973) rand_m -= 9871;
    if ((rand_ia += 1907) >= 99991) rand_ia -= 89989;
    if ((rand_ib += 73939) >= 224729) rand_ib -= 96233;
...
void CreateUUID(uuid_t* uuid)
{
    ...
    /* Construct a uuid with the information we've gathered * plus a few constants. */
...
void CreateStringUUID(uuid_t u, char* uuid)
{
    int i = 0, len = 0;
    char temp[20] = { 0 };
    for (; i < 6; i++) {
        len += sprintf(temp + len, "%2.2x", u.node[i]);
    }
    sprintf(uuid, "%8.8x%4.4x%4.4x%2.2x%2.2x%s", (unsigned int)u.time_low, u.time_mid,
            u.time_hi_and_version, u.clock_seq_hi_and_reserved, u.clock_seq_low, temp);
    return;
}
