...
lib_version=`svn info | cat -n | awk '{if($1==3)print $3}' |  awk -F'/v' '{print $2}' | awk -F'/' '{print $1}'`
echo $lib_version

cp -rf CMakeLists-static.txt CMakeLists.txt

for file in ../../../platform/toolchain/linux/*
do
	if test -d $file
	then
		echo $file
